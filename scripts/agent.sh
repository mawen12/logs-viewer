...
if [[ $? != 0 ]]; then
  echo "E:1:gawk (GNU Awk) is a requirement, but not found on the system. Please install it, then retry" 1>&2
  exit 1
fi

os_kind="$(detect_os_kind)"
if [[ $? != 0 ]]; then
  echo "E:1:unknown kernel name $(uname -s)" 1>&2
  exit 1
fi

if [[ "$logfile" == "" ]]; then
  echo "E:1:--logfile is required. Specify the log file manually" 1>&2
  exit 1
fi

command="$1"
if [[ "${command}" == "" ]]; then
  echo "E:1:command is required" 1>&2
  exit 1
fi
...
case "${command}" in 
  ...
  logstream_info)
    ...
    if [[ $? == 0 ]]; then
      echo "X:tz:$host_timezone"
    else 
      echo "E:1:failed to detect host timezone" 1>&2
    fi

    if [ ! -e ${logfile} ]; then
      echo "E:1:${logfile} does not exist" 1>&2
      exit 1
    fi

    if [ ! -r ${logfile} ]; then
      echo "E:1:${logfile} exists but is not readable, check your permissions" 1>&2
      exit 1
    fi
    ...
    if [ -s ${logfile} ]; then
      ...
      echo "X:lastLogline:$last_line"
      echo "X:firstLogline:$first_line"
    fi

    exit 0
    ;;
  *)
    echo "E:1:invalid command ${command}" 1>&2
    exit 1
esac

## queryLogs 必须指定索引文件，此时需要完全重构索引文件
if [[ $indexfile == "" ]]; then
  echo "E:1:-c|--index-file is not set" 1>&2
  exit 1
fi
    ...
    if [[ "$from" != "" ]]; then
      ...
      if [[ "$from_result" == "before" ]]; then
        ...
        if [[ "$from_linenr" == "" || "$from_bytenr" == "" ]]; then
          echo "E:1:from_result is found but from_bytenr and/or from_linenr is empty" 1>&2
          exit 1
        fi
        ...
        echo "E:invalid from_result: $from_result" 1>&2
        exit 1  
      fi
    fi
    ...
    if [[ "$to" != "" ]]; then
      ...
      if [[ "$to_result" == "after" ]]; then
        ...
        if [[ "$to_linenr" == "" || "$to_bytenr" == "" ]]; then
          echo "E:1:to_result is found but to_bytenr and/or to_linenr is empty" 1>&2
          exit 1
        fi
        ...
        echo "E:1:invalid to_result: $to_result" 1>&2
        exit 1    
      fi
    fi
  fi
